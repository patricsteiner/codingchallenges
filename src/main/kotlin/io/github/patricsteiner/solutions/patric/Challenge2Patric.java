...
public class Challenge2Patric {
    ...
    int[] splitSum(int sum, int[] allObjektArts) {
        ...
        for (int objektArt : allObjektArts) {
            if (seen.contains(sum - objektArt)) {
                return new int[]{sum - objektArt, objektArt};
            }
            seen.add(objektArt);
        }
        return null;
